...
-- 
-- Usage:
--   sqlcmd -S SERVER -d DATABASE -i sql/utils/copy-poc-etl-to-old-poc.sql
-- 
...
PRINT '============================================================';
PRINT 'COPY SCHEMA: poc_etl → old_poc';
PRINT '============================================================';
PRINT '';
...
IF NOT EXISTS (SELECT * FROM sys.schemas WHERE name = 'old_poc')
BEGIN
    EXEC('CREATE SCHEMA [old_poc]');
    PRINT '   ✅ Schema [old_poc] created';
END
ELSE
BEGIN
    PRINT '   ℹ️  Schema [old_poc] already exists';
END
...
-- Cursor to iterate through all tables in poc_etl schema
...
BEGIN
    ...
    BEGIN TRY
        ...
        SET @sql = N'SELECT @count = COUNT(*) FROM [poc_etl].[' + @tableName + ']';
        ...
        SET @sql = N'
        IF OBJECT_ID(''[old_poc].[' + @tableName + ']'', ''U'') IS NOT NULL
            DROP TABLE [old_poc].[' + @tableName + '];';
        EXEC sp_executesql @sql;
        
        -- Copy table structure and data using SELECT INTO
        SET @sql = N'
        SELECT * INTO [old_poc].[' + @tableName + ']
        FROM [poc_etl].[' + @tableName + '];';
        EXEC sp_executesql @sql;
        
        -- Verify row count
        SET @sql = N'SELECT @count = COUNT(*) FROM [old_poc].[' + @tableName + ']';
...
PRINT '============================================================';
PRINT '';
PRINT 'Schema [old_poc] is ready with all data from [poc_etl]';
PRINT '';
