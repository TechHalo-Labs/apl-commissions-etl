...
SELECT
    ...
    TRY_CAST(REPLACE(JSON_VALUE(pdk.ConfigJson, '$[0].brokerId'), 'P', '') AS BIGINT) AS BrokerId,
...
LEFT JOIN [etl].[stg_brokers] b ON b.Id = TRY_CAST(REPLACE(JSON_VALUE(pdk.ConfigJson, '$[0].brokerId'), 'P', '') AS BIGINT);
