...
-- =============================================================================
DROP TABLE IF EXISTS [etl].[ref_active_groups];
CREATE TABLE [etl].[ref_active_groups] (
...
DROP TABLE IF EXISTS [etl].[ref_active_certificates];
CREATE TABLE [etl].[ref_active_certificates] (
...
DROP TABLE IF EXISTS [etl].[ref_active_brokers];
CREATE TABLE [etl].[ref_active_brokers] (
    BrokerId NVARCHAR(50) NOT NULL,
    [Source] NVARCHAR(50),
    CONSTRAINT PK_ref_active_brokers PRIMARY KEY (BrokerId, [Source])
);

DROP TABLE IF EXISTS [etl].[ref_active_schedules];
CREATE TABLE [etl].[ref_active_schedules] (
...
INSERT INTO [etl].[ref_active_groups] (GroupNumber, GroupName, [Source])
SELECT DISTINCT
    GroupNumber,
...
INNER JOIN [etl].[input_commission_details] cd ON ci.CertificateId = cd.CertificateId
WHERE ci.GroupId IS NOT NULL AND ci.GroupId <> ''
  AND NOT EXISTS (
      SELECT 1 FROM [etl].[ref_active_groups] r 
      WHERE r.GroupNumber = ci.GroupId
  );

PRINT 'Groups from commissions: ' + CAST(@@ROWCOUNT AS VARCHAR);

SELECT COUNT(*) AS active_groups FROM [etl].[ref_active_groups];
...
INSERT INTO [etl].[ref_active_certificates] (CertificateId, GroupNumber, [Source])
SELECT DISTINCT
...
SELECT DISTINCT
    ...
    'commissions' AS [Source]
FROM [etl].[input_commission_details] cd
LEFT JOIN [etl].[input_certificate_info] ci ON cd.CertificateId = ci.CertificateId
WHERE cd.CertificateId > 0
  AND NOT EXISTS (
      SELECT 1 FROM [etl].[ref_active_certificates] r 
      WHERE r.CertificateId = CAST(cd.CertificateId AS NVARCHAR(100))
  );

PRINT 'Certificates from commissions: ' + CAST(@@ROWCOUNT AS VARCHAR);

SELECT COUNT(*) AS active_certificates FROM [etl].[ref_active_certificates];
...
INSERT INTO [etl].[ref_active_brokers] (BrokerId, [Source])
SELECT DISTINCT
...
WHERE CAST(ci.CertificateId AS NVARCHAR(100)) IN (SELECT CertificateId FROM [etl].[ref_active_certificates])
  ...
  AND NOT EXISTS (
      SELECT 1 FROM [etl].[ref_active_brokers] r 
...
SELECT COUNT(*) AS active_brokers FROM [etl].[ref_active_brokers];
...
INSERT INTO [etl].[ref_active_schedules] (ScheduleName, [Source])
SELECT DISTINCT
...
SELECT 
    ...
    (SELECT COUNT(*) FROM [etl].[ref_active_groups]) AS groups,
    (SELECT COUNT(*) FROM [etl].[ref_active_certificates]) AS certificates,
    (SELECT COUNT(*) FROM [etl].[ref_active_brokers]) AS brokers,
    (SELECT COUNT(*) FROM [etl].[ref_active_schedules]) AS schedules;
