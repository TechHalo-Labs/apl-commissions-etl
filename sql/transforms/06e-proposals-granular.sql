...
DECLARE @input_certs INT = (SELECT COUNT(*) FROM [$(ETL_SCHEMA)].[cert_split_configs_remainder3]);
DECLARE @input_groups INT = (SELECT COUNT(DISTINCT GroupId) FROM [$(ETL_SCHEMA)].[cert_split_configs_remainder3]);
...
DROP TABLE IF EXISTS [$(ETL_SCHEMA)].[granular_keys];
...
INTO [$(ETL_SCHEMA)].[granular_keys]
FROM [$(ETL_SCHEMA)].[cert_split_configs_remainder3]
...
LEFT JOIN [$(ETL_SCHEMA)].[stg_groups] g ON g.Id = CONCAT('G', gk.GroupId)
LEFT JOIN [$(ETL_SCHEMA)].[stg_brokers] b ON b.ExternalPartyId = REPLACE(REPLACE(JSON_VALUE(gk.ConfigJson, '$[0].brokerId'), 'P', ''), ' ', '');
